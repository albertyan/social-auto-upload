...
- **首装流程**（`[Code]` / `[Run]` 段）：
  ...
  6. 完成页提示。
- **浏览器内核下载时机（任务 #26 修订，覆盖 v1.3 定案）**：**安装时自动下载**——服务注册/启动成功后由 `post_install.bat` 执行 `sau.exe browser install`（复用 §8.7 三层下载：npmmirror → 官方 → 镜像轮换、60 秒卡死换源）；**已装跳过**（校验 `INSTALLATION_COMPLETE`/可执行文件，覆盖升级不重复下载约 170MB）；弱网总时限 **20 分钟**，超时按失败处理；**下载失败不阻断安装**（§17 阶段 5），进度与结果写 `post_install.log` 及 `%ProgramData%\SAU\logs\browser_install.log`，控制台账号页内核缺失指引卡与 `doctor` 引导手动重试。**交付预期**：弱网环境安装耗时随镜像源速度可达数分钟至 20 分钟上限；失败后用户路径＝控制台账号页指引卡／`sau.exe browser install` 手动命令／离线包 `--from-file`。修订缘由（用户真机拍板 2026-08-26）：首启按需触发对普通用户不可操作（无入口、无法确认完成），真机排障被该缺口反复卡住。
...
与 §17 安装失败矩阵阶段 5（浏览器内核下载失败不阻断安装）呼应；该命令定义已落到 3.2 子命令规格表；安装时机见 7.3（任务 #26 修订：**安装时自动下载、失败不阻断、已装跳过**，覆盖 v1.3「首启按需触发」）。
...
*本文档初版为任务 #5 产出，任务 #7 更新为 v1.1：本地端口整合为 5409（见 4.4）、冲突 1 定案为包装层自建并扩写控制台实现方案（见第 6 章）。任务 #9 更新为 v1.2：逐项讨论定案——补齐 10 项关键遗漏（新增 §3.6/3.7/3.8 与第 13~17 章）、修正 6 处矛盾、取消旧 4-exe 过渡链路（旧版从未发布）。任务 #11 更新为 v1.3：修正验证复查发现的 7 处问题，落档打包体积优化（8.6）与浏览器内核下载三层方案（8.7）两项新定案。任务 #26 更新为 v1.4：浏览器内核下载时机改为安装时自动下载（覆盖 v1.3「首启按需触发/不在安装器内下载」，§7.3/§8.7/§17 同步修订）；内核体积口径按双组件实测更正（8.6：省约 295MB，完整内核 zip ~173MB + headless shell zip ~114MB）。讨论记录部分为全过程实录（用户原话、备选方案、每轮决策均完整保留，其中 5410 为历史原话，均按 4.4 端口决策理解）；设计章节以讨论结论及后续定案为准。*
