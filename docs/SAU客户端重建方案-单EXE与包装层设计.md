...
| 项目 | 内容 |
| --- | --- |
| 文档版本 | v1.4（任务 #26：浏览器内核下载时机改为安装时自动下载，覆盖 v1.3「不在安装器内下载」定案；用户真机拍板 2026-08-26） |
...
- 判绿收紧（任务 #3/#4 落地修订）：在线判定纳入 `/status` 的 `token_status`——200 且 `ws_connected` 且 `token_status` 为 `ok` 才判在线（字段缺失按 `ok` 处理，兼容旧版服务响应）；`expired`/`suspended`/`unbound` 归离线（图标转灰 + 离线提示），连接态 tooltip 同步体现凭证问题与处理指引；
- 图标自愈（任务 #3/#4 落地修订）：启动时预缓存双图（在线绿/离线灰，Pillow 色块），每轮轮询**无条件重赋**缓存位图（原「仅翻转时重赋」的边沿分支已删除）——某轮 `Shell_NotifyIcon` 静默失败导致图标灰死时，最多一个轮询周期后被下一轮重赋覆盖自愈。
...
- **首装流程**（`[Code]` / `[Run]` 段）：
  ...
  6. 完成页提示。
- **浏览器内核下载时机（任务 #26 修订，覆盖 v1.3 定案）**：**安装时自动下载**——服务注册/启动成功后由 `post_install.bat` 执行 `sau.exe browser install`（复用 §8.7 三层下载：npmmirror → 官方 → 镜像轮换、60 秒卡死换源）；**已装跳过**（校验 `INSTALLATION_COMPLETE`/可执行文件，覆盖升级不重复下载约 170MB）；弱网总时限 **20 分钟**，超时按失败处理；**下载失败不阻断安装**（§17 阶段 5），进度与结果写 `post_install.log` 及 `%ProgramData%\SAU\logs\browser_install.log`，控制台账号页内核缺失指引卡与 `doctor` 引导手动重试。**执行位置修订（任务 #3/#4 落地，任务 #10 再修订）**：内核下载已从 `post_install.bat` 移至安装向导（sau.iss `CurStepChanged`）：`post_install.bat` 成功收尾后以后台隐藏窗口（SW_HIDE）`Exec` `sau.exe browser install`；**进度反馈方式（任务 #10）**：原「可见控制台窗口」方案已证实失效（sau.exe 为 GUI 子系统、无控制台），改为**安装页面进度条**——后台下载 + 进度文件轮询：Python 侧经 `--progress-file` 将进度（stage/percent/seq 心跳）写入 `%ProgramData%\SAU\download_progress.ini`，安装向导 500ms 轮询并实时刷新 `WizardForm.ProgressGauge` 与状态文案（done 终态 + seq 停滞判崩溃；下载期间禁用取消按钮，纯 ASCII 值域适配 GBK 解析）；携带 /SILENT、/VERYSILENT 或 /SUPPRESSMSGBOXES 任一参数时维持隐藏阻塞等待；下载失败**不阻断安装收尾**，弹窗提示手动补救（`sau.exe browser install` 或离线包 `--from-file`）；`post_install.bat` 保持纯 ASCII 且退出码矩阵（0/11/12）语义不变。**交付预期**：弱网环境安装耗时随镜像源速度可达数分钟至 20 分钟上限；失败后用户路径＝控制台账号页指引卡／`sau.exe browser install` 手动命令／离线包 `--from-file`。修订缘由（用户真机拍板 2026-08-26）：首启按需触发对普通用户不可操作（无入口、无法确认完成），真机排障被该缺口反复卡住。
- **覆盖升级说明**：覆盖升级场景仍需停服 + 等待进程退出与文件句柄释放（PrepareToInstall，见 4.2）；首次安装无存量服务，不涉及该步骤。停托盘同样先礼后兵（任务 #3/#4）：`sau.exe tray-exit` 优雅退出（走 `icon.stop()` 发 NIM_DELETE 防通知区幽灵图标）在前，超时或旧版无该命令时 `taskkill` 兜底，时序与卸载一致（见 13.1）；静默升级（/SILENT、/VERYSILENT 或 /SUPPRESSMSGBOXES 任一）下内核下载窗口隐藏，行为与现状一致。
...
| ④ | 浏览器内核不随包、运行时下载（已设计，见 7.3 与 8.7） | 省约 295MB（双组件 zip 合计：完整内核 ~173MB + headless shell ~114MB，任务 #26 实测口径） | 依赖浏览器下载方案可靠性（见 8.7） |
| ⑤ | Inno Setup LZMA2 ultra（已设计） | 已生效 | 无 |
| ⑥ | UPX 压缩 | 再省 10~20% | **不采用**：杀软误报率显著上升，与首发无签名叠加风险大 |

**体积目标**：安装包 **≤100MB（不含浏览器内核）**，写入 16.1 打包冒烟清单作为验收项。

**二期可选实验项**：若确认所有平台发布与登录均可 headless，可只下载 chromium-headless-shell（zip 约 114MB，比完整内核 zip 约 173MB 省约三分之一；任务 #26 实测口径）；需逐平台实测反检测兼容性，不阻塞一期。

### 8.7 浏览器内核下载三层方案（v1.3 定案，任务 #26 修订为直链自管下载）

**根因**：patchright/playwright 默认从微软 CDN（azureedge.net）下载内核，国内网络常缓慢或中途挂起（「卡着」不报错，体验最差）。

**三层设计**：

1. **镜像源策略**：首选 npmmirror 的 Chrome for Testing 直链自管下载（任务 #26 实测：`PLAYWRIGHT_DOWNLOAD_HOST` 的 playwright 镜像路径对新版内核返回 404，故改为 `registry.npmmirror.com/-/binary/chrome-for-testing/{version}/win64/chrome-win64.zip` 直链，字节级进度完全可控），官方源经 patchright 驱动作回退；
2. **下载可靠性**：显示进度百分比与速度；60 秒无字节进展判定卡死 → 自动断开重试 → 源间切换（直链镜像→官方→直链镜像轮换，共 3 轮；直链轮以套接字读超时 30s 作卡死保护）；支持重新发起；
3. **离线兜底**：提供手动安装入口——用户从任意渠道下载内核 zip，放入 `%ProgramData%\SAU\browsers\`，执行 `sau.exe browser install --from-file <zip>` 本地解压安装；覆盖公司内网 / 无外网环境。

与 §17 安装失败矩阵阶段 5（浏览器内核下载失败不阻断安装）呼应；该命令定义已落到 3.2 子命令规格表；安装时机见 7.3（任务 #26 修订：**安装时自动下载、失败不阻断、已装跳过**，覆盖 v1.3「首启按需触发」）。执行位置任务 #3/#4 落地修订（任务 #10 再修订）：由 `post_install.bat` 移至安装向导内以后台隐藏窗口执行，交互模式以安装页面进度条展示进度（后台下载 + 进度文件轮询；原可见控制台窗口方案因 sau.exe 为 GUI 子系统无控制台而失效）；携带 /SILENT、/VERYSILENT 或 /SUPPRESSMSGBOXES 任一参数时维持隐藏阻塞等待；失败弹窗不阻断、可手动补救，见 7.3。
...
| 27 | 逐项定案（任务 #9） | 卸载策略 | 不补；六步时序 | **六步卸载 + 数据默认保留 + 多用户残留接受现状**；任务 #3/#4 落地修订：凭证（`credential.bin`/`local_token.bin`）**始终删除**，其余数据卸载前弹框询问、默认保留，卸载进度窗勾选框已移除；静默卸载默认保留数据但凭证仍删 | 见第 13 章 |
...
1. 停托盘进程——**先礼后兵**（任务 #3/#4 修订）：`sau.exe tray-exit` 优雅退出（命名事件 `SAUTrayExitEvent` 通知，托盘走 `icon.stop()` 发 NIM_DELETE，防通知区残留幽灵图标）在前；超时托盘仍在或旧版无该命令（事件不存在秒返回 0，行为等同现状）时再 `taskkill` 兜底。覆盖升级路径（PrepareToInstall）同样先礼后兵，时序一致（见 7.3）；
...
5. Inno 删除 `{app}` 目录；
6. 数据目录（`%ProgramData%\SAU`）处置：凭证文件（`credential.bin`/`local_token.bin`）**始终删除**；其余数据卸载前弹框询问、默认保留（见 13.2）。

### 13.2 数据保留策略（凭证必删，其余默认保留；任务 #3/#4 修订，覆盖原「勾选框默认不勾」口径）

凭证文件（`credential.bin`/`local_token.bin`，DPAPI 加密的 Agent Token 与本地令牌）**始终删除**：卸载阶段（`usPostUninstall`）逐个删除并校验，被占用时等待重试（500ms × 3），仍失败则弹窗报错——敏感凭证不得静默跳过，且重装不得复用旧 Token。其余数据（cookies / 任务库等，`%ProgramData%\SAU`）**卸载前弹框询问**（是/否，默认「否」＝保留），选「是」才整删数据目录，重装后可恢复登录态。原卸载进度窗勾选框已整体移除（进度窗一闪而过来不及勾）；改为卸载前弹框询问，时间充裕。行为变更注意：**静默卸载（`/SILENT`/`/VERYSILENT`）跳过弹框，默认保留其余数据，但凭证仍始终删除**（`/SUPPRESSMSGBOXES` 场景按默认「否」处理）。**自动化/运维口径（任务 #6 评审修复）**：自动化/运维卸载必须携带 `/SILENT`、`/VERYSILENT` 或 `/SUPPRESSMSGBOXES` 之一（三参数任一即隐藏内核下载窗口并跳过询问，见 `SauSilentMode`）；交互卸载会弹「是否删除本地数据」确认框，属预期设计。

### 13.3 多用户自启残留（接受现状）

卸载只清理**执行卸载的当前用户**的 `HKCU\...\Run\SAUTray` 项；其他用户会话下的残留自启项不跨用户清理，接受现状，并在卸载说明中注明（残留托盘进程下次启动时会因服务不存在而仅显示离线图标，无功能危害）。

### 13.4 安装/卸载界面中文化（任务 #3/#4 落地）

`[Languages]` 仅保留简体中文一条（`Languages\ChineseSimplified.isl` 已入库，Inno Setup 6.5.0+ 官方翻译文件），避免启动时弹语言选择对话框；静默安装可用 `/LANG=chinesesimplified` 显式指定。不兼容说明：`/LANG=english` 自本版起失效（英文语言条目已移除），仅支持 `/LANG=chinesesimplified`。卸载/安装向导提示、数据询问弹框与失败弹窗均为中文文案；另有实测偏差留痕：本机 Inno 6.7.3 不存在 `WizardSilentMode`/`UninstallSilent`，脚本以自定义 `SauSilentMode()`（解析 `GetCmdTail` 匹配 `/silent`、`/verysilent`、`/suppressmsgboxes`，三参数任一即命中「隐藏交互/静默」口径；任务 #6 评审修复追加 `/SUPPRESSMSGBOXES`）替代。
...
| 5 | 浏览器内核下载失败（安装时自动下载，任务 #26 修订） | **不阻断安装**：留痕 `post_install.log` / `browser_install.log`，控制台账号页指引卡 + `doctor` 引导 `sau.exe browser install` 手动重试 |
...
*本文档初版为任务 #5 产出，任务 #7 更新为 v1.1：本地端口整合为 5409（见 4.4）、冲突 1 定案为包装层自建并扩写控制台实现方案（见第 6 章）。任务 #9 更新为 v1.2：逐项讨论定案——补齐 10 项关键遗漏（新增 §3.6/3.7/3.8 与第 13~17 章）、修正 6 处矛盾、取消旧 4-exe 过渡链路（旧版从未发布）。任务 #11 更新为 v1.3：修正验证复查发现的 7 处问题，落档打包体积优化（8.6）与浏览器内核下载三层方案（8.7）两项新定案。任务 #26 更新为 v1.4：浏览器内核下载时机改为安装时自动下载（覆盖 v1.3「首启按需触发/不在安装器内下载」，§7.3/§8.7/§17 同步修订）；内核体积口径按双组件实测更正（8.6：省约 295MB，完整内核 zip ~173MB + headless shell zip ~114MB）。任务 #3/#4 落地同步（v1.4+）：卸载数据处置口径改为凭证（`credential.bin`/`local_token.bin`）始终删除、其余数据卸载前弹框询问默认保留、勾选框移除（§13.2/决策表行 27）；浏览器内核下载执行位置由 `post_install.bat` 移至安装向导（可见控制台窗口、静默隐藏、失败不阻断，§7.3/§8.7）；卸载与覆盖升级停托盘改为先礼后兵（`sau.exe tray-exit` 优雅退出在前，`taskkill` 兜底，§13.1/§7.3）；托盘判绿纳入 `token_status`、图标预缓存双图每轮重赋自愈（§5.2）；安装/卸载界面中文化（`ChineseSimplified.isl` 入库，§13.4）。讨论记录部分为全过程实录（用户原话、备选方案、每轮决策均完整保留，其中 5410 为历史原话，均按 4.4 端口决策理解）；设计章节以讨论结论和后续定案为准。*
